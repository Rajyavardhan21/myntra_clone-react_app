...
export default class productDetail extends Component {
    ...
    //       for (let j = 0; j < totalRating - parseInt(data); j++) {
    //         stars.push(<i className="far fa-star" key={parseInt(data) + j}></i>);
    //       }
    //     }
    ...
    //         // Half-filled star
    //         if (data % 1 !== 0) {
    //             stars.push(<i className="fas fa-star-half-alt" key={Math.floor(data) + 1}></i>);
    //         }

    //         // Empty stars
    //         for (let j = Math.ceil(data); j < totalRating; j++) {
    //             stars.push(<i className="far fa-star" key={j}></i>);
    //         }
    //     }
        ...
        return (
            <div style={{ marginTop: 120, display: 'flex' }}>
                ...
                return (
                    ...
                    {/* {this.productRating(this.state.productData.product_rating).map((item) => {
                        return item;
                    })} */}
                    {this.productRating(this.state.productData.product_rating)}
